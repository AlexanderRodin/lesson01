package lesson03;

import java.util.Random;
import java.util.Scanner;

public class XOGame {

public static final char HUMAN_DOT = 'X';
public static final char PC_DOT = 'O';
public static final char EMPTY_DOT = '_';

public static final Scanner SCANNER = new Scanner(System.in);

public static final Random RANDOM = new Random ();

    public static char [] [] map;
    public static int mapSizeX;
    public static int mapSizeY;

    public static void initMap () {
        mapSizeX = 5;
        mapSizeY = 5;
        map = new char[mapSizeY] [mapSizeX];

        for (int y = 0; y <mapSizeY; y ++) {
            for (int x = 0; x <mapSizeX; x ++) {
                map [y] [x] = EMPTY_DOT;
            }
        }
    }

    public static void printMap () {
        for (int y = 0; y <mapSizeY; y ++) {
            for (int x = 0; x <mapSizeX; x ++) {
                System.out.print (map [y] [x] + "|");
            }
            System.out.println ();
        }
        System.out.println ();
    }

    public static void humanTurn () {
        int x;
        int y;

        do {
            System.out.println ("Полноприводная информация:");
            x = SCANNER.nextInt () - 1;
            y = SCANNER.nextInt () - 1;

        } while (! isValidCell (y, x) ||! isEmptyCell (y, x));
        map [y] [x] = HUMAN_DOT;
    }

    public static void aiTurn () {
        int x;
        int y;

        do {
            x = RANDOM.nextInt (mapSizeX);
            y = RANDOM.nextInt (mapSizeY);
        } while (! isEmptyCell (y, x));
        map [y] [x] = PC_DOT;
    }


    public static boolean isValidCell (int y, int x) {
        return x >= 0 && x <mapSizeX && y >= 0 && y <mapSizeY;
    }

    public static boolean isEmptyCell (int y, int x) {
        return map [y] [x] == EMPTY_DOT;
    }

    public static boolean checkWin (char inboxChar) {
//        if (map [0] [0] == inboxChar && map [0] [1] == inboxChar && map [0] [2] == inboxChar) return true;
//        if (map [1] [0] == inboxChar && map [1] [1] == inboxChar && map [1] [2] == inboxChar) return true;
//        if (map [2] [0] == inboxChar && map [2] [1] == inboxChar && map [2] [2] == inboxChar) return true;
//
//        if (map [0] [0] == inboxChar && map [1] [0] == inboxChar && map [2] [0] == inboxChar) return true;
//        if (map [0] [1] == inboxChar && map [1] [1] == inboxChar && map [2] [1] == inboxChar) return true;
//        if (map [0] [2] == inboxChar && map [1] [2] == inboxChar && map [2] [2] == inboxChar) return true;
//
//        if (map [0] [0] == inboxChar && map [1] [1] == inboxChar && map [2] [2] == inboxChar) return true;
//        if (map [0] [2] == inboxChar && map [1] [1] == inboxChar && map [2] [0] == inboxChar) return true;
//        return false;

        int count=0;

        //проверка по вертикали (на данный момент считает не корректно)
        // Проверку по смежным вертикалям реолизовать пока не получается, буду признателен если на толкнете на мысль))
        // Магическое число 4 в сравнении циклов переедет в статичную переменную в начале программы, подразумевает количество нужных шагов для победы

        for(int i=0;i < map.length-1;i++)

            for (int j = 0; j < map[i].length - 1; j++)
                if (count == 4) return true;
                else if (map[i][j] == inboxChar) count += 1;



        // Проверка по диагонали

        count = 0;
        for(int i=1;i<map.length;i++)
             for (int j = 1; j < i + 1; j++)
                if (map[i][j] == inboxChar) count += 1;
        if (count == 4) return true;

        count = 0;

        for(int i=0;i<map.length-1;i++)
              for (int j = 0; j < i + 1; j++)
                  if (map[i][j] == inboxChar) count += 1;
        if (count == 4)return true;

        return false;





    }

    public static boolean isMapFull () {
        for (int y = 0; y <mapSizeY; y ++) {
            for (int x = 0; x <mapSizeX; x ++) {
                if (map [y] [x] == EMPTY_DOT) return false;
            }
        }
        return true;
    }

    public static void main (String [] args) {
        initMap ();
        printMap ();
        while (true) {
            humanTurn ();
            printMap ();

            if (checkWin (HUMAN_DOT)) {
                System.out.println ("Победа человека !!!");
                break;
            }
            if (isMapFull ()) {
                System.out.println ("Хрен !!!");
                break;
            }

            aiTurn ();
            printMap ();
            if (checkWin (PC_DOT)) {
                System.out.println ("ИИ победил !!! ^ (((((");
                break;
            }
            if (isMapFull ()) {
                System.out.println ("Хрен !!!");
                break;
            }
        }
    }
}
